...
load("/Users/dbendet/Documents/Code/r/hockey_analytics/source-data/nhlscrapr-20122013.RData")
...
distinct(select(s1213, etype))

s1213 = filter(s1213, gcode < 30000)
...
View(arrange(s1213 %>% group_by(distance) %>% summarize(count = n()), desc(count)))

# shot success rate by section
View(arrange(filter(s1213, etype == 'SHOT' | etype == 'GOAL') %>% group_by(new.loc.section) %>% summarize(distance = mean(distance, na.rm = TRUE), shotrate = sum(ifelse(etype == 'GOAL', 1, 0))/sum(ifelse(etype == 'GOAL' | etype == 'SHOT', 1, 0)), counts = n()), desc(shotrate)))



# adding all above into results table 

s1213results = select(filter(s1213, 
  reverse_rank == 1), 
  season, 
  gcode, 
  period, 
...
s1213home = s1213results

s1213home = mutate(s1213home, team = hometeam)

s1213away = s1213results

s1213away = mutate(s1213away, team = awayteam)

s1213teamresults = rbind_all(list(s1213home, s1213away))

View(arrange(s1213teamresults, gcode))

s1213teamresults = s1213teamresults %>%  
group_by(team) %>%
...
x = filter(s1213teamresults, gamerank < 42)
y = filter(s1213teamresults, gamerank > 42)
...
s1213teamresults = rbind_all(list(x, y))


s1213corr = s1213teamresults %>%
group_by(team) %>%
...
s1213finalcorr = as.data.frame(cor(select(
  s1213corr, 
  wins2, 
  wins1, 
  wins11,
