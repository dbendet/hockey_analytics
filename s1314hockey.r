...
load("/Users/dbendet/Documents/Code/r/hockey_analytics/source-data/nhlscrapr-20132014.RData")
...
distinct(select(s1314, etype))

s1314 = filter(s1314, gcode < 30000)
...
View(arrange(s1314 %>% group_by(distance) %>% summarize(count = n()), desc(count)))

# shot success rate by section
View(arrange(filter(s1314, etype == 'SHOT' | etype == 'GOAL') %>% group_by(new.loc.section) %>% summarize(distance = mean(distance, na.rm = TRUE), shotrate = sum(ifelse(etype == 'GOAL', 1, 0))/sum(ifelse(etype == 'GOAL' | etype == 'SHOT', 1, 0)), counts = n()), desc(shotrate)))



# adding all above into results table 

s1314results = select(filter(s1314, 
  reverse_rank == 1), 
  season, 
  gcode, 
  period, 
...
s1314home = s1314results

s1314home = mutate(s1314home, team = hometeam)

s1314away = s1314results

s1314away = mutate(s1314away, team = awayteam)

s1314teamresults = rbind_all(list(s1314home, s1314away))

View(arrange(s1314teamresults, gcode))

s1314teamresults = s1314teamresults %>%  
group_by(team) %>%
...
x = filter(s1314teamresults, gamerank < 42)
y = filter(s1314teamresults, gamerank > 42)
...
s1314teamresults = rbind_all(list(x, y))


s1314corr = s1314teamresults %>%
group_by(team) %>%
...
s1314finalcorr = as.data.frame(cor(select(
  s1314corr, 
  wins2, 
  wins1, 
  wins11,
