...
load("/Users/dbendet/Documents/Code/r/hockey_analytics/source-data/nhlscrapr-20072008.RData")
...
distinct(select(s0708, etype))

s0708 = filter(s0708, gcode < 30000)
...
View(arrange(s0708 %>% group_by(distance) %>% summarize(count = n()), desc(count)))

# shot success rate by section
View(arrange(filter(s0708, etype == 'SHOT' | etype == 'GOAL') %>% group_by(new.loc.section) %>% summarize(distance = mean(distance, na.rm = TRUE), shotrate = sum(ifelse(etype == 'GOAL', 1, 0))/sum(ifelse(etype == 'GOAL' | etype == 'SHOT', 1, 0)), counts = n()), desc(shotrate)))



# adding all above into results table 

s0708results = select(filter(s0708, 
  reverse_rank == 1), 
  season, 
  gcode, 
  period, 
...
s0708home = s0708results

s0708home = mutate(s0708home, team = hometeam)

s0708away = s0708results

s0708away = mutate(s0708away, team = awayteam)

s0708teamresults = rbind_all(list(s0708home, s0708away))

View(arrange(s0708teamresults, gcode))

s0708teamresults = s0708teamresults %>%  
group_by(team) %>%
...
x = filter(s0708teamresults, gamerank < 42)
y = filter(s0708teamresults, gamerank > 42)
...
s0708teamresults = rbind_all(list(x, y))


s0708corr = s0708teamresults %>%
group_by(team) %>%
...
s0708finalcorr = as.data.frame(cor(select(
  s0708corr, 
  wins2, 
  wins1, 
  wins11,
