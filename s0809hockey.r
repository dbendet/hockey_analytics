...
load("/Users/dbendet/Documents/Code/r/hockey_analytics/source-data/nhlscrapr-20082009.RData")
...
distinct(select(s0809, etype))

s0809 = filter(s0809, gcode < 30000)
...
View(arrange(s0809 %>% group_by(distance) %>% summarize(count = n()), desc(count)))

# shot success rate by section
View(arrange(filter(s0809, etype == 'SHOT' | etype == 'GOAL') %>% group_by(new.loc.section) %>% summarize(distance = mean(distance, na.rm = TRUE), shotrate = sum(ifelse(etype == 'GOAL', 1, 0))/sum(ifelse(etype == 'GOAL' | etype == 'SHOT', 1, 0)), counts = n()), desc(shotrate)))



# adding all above into results table 

s0809results = select(filter(s0809, 
  reverse_rank == 1), 
  season, 
  gcode, 
  period, 
...
s0809home = s0809results

s0809home = mutate(s0809home, team = hometeam)

s0809away = s0809results

s0809away = mutate(s0809away, team = awayteam)

s0809teamresults = rbind_all(list(s0809home, s0809away))

View(arrange(s0809teamresults, gcode))

s0809teamresults = s0809teamresults %>%  
group_by(team) %>%
...
x = filter(s0809teamresults, gamerank < 42)
y = filter(s0809teamresults, gamerank > 42)
...
s0809teamresults = rbind_all(list(x, y))


s0809corr = s0809teamresults %>%
group_by(team) %>%
...
s0809finalcorr = as.data.frame(cor(select(
  s0809corr, 
  wins2, 
  wins1, 
  wins11,
