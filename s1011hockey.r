...
load("/Users/dbendet/Documents/Code/r/hockey_analytics/source-data/nhlscrapr-20102011.RData")
...
distinct(select(s1011, etype))

s1011 = filter(s1011, gcode < 30000)
...
View(arrange(s1011 %>% group_by(distance) %>% summarize(count = n()), desc(count)))

# shot success rate by section
View(arrange(filter(s1011, etype == 'SHOT' | etype == 'GOAL') %>% group_by(new.loc.section) %>% summarize(distance = mean(distance, na.rm = TRUE), shotrate = sum(ifelse(etype == 'GOAL', 1, 0))/sum(ifelse(etype == 'GOAL' | etype == 'SHOT', 1, 0)), counts = n()), desc(shotrate)))



# adding all above into results table 

s1011results = select(filter(s1011, 
  reverse_rank == 1), 
  season, 
  gcode, 
  period, 
...
s1011home = s1011results

s1011home = mutate(s1011home, team = hometeam)

s1011away = s1011results

s1011away = mutate(s1011away, team = awayteam)

s1011teamresults = rbind_all(list(s1011home, s1011away))

View(arrange(s1011teamresults, gcode))

s1011teamresults = s1011teamresults %>%  
group_by(team) %>%
...
x = filter(s1011teamresults, gamerank < 42)
y = filter(s1011teamresults, gamerank > 42)
...
s1011teamresults = rbind_all(list(x, y))


s1011corr = s1011teamresults %>%
group_by(team) %>%
...
s1011finalcorr = as.data.frame(cor(select(
  s1011corr, 
  wins2, 
  wins1, 
  wins11,
